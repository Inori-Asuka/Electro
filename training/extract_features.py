    ...
    def extract_from_loader(loader, dataset, dataset_name):
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(tqdm(loader, desc=f"Extract {dataset_name} Feature")):
                ...
                features = model.extract_features(images)  # 可能是 [B, embed_dim] 或 [B, N, embed_dim]
                
                # 处理不同形状的特征
                if isinstance(features, torch.Tensor):
                    features_np = features.cpu().numpy()
                    # 如果是3D张量 [B, N, C]，需要展平或池化
                    if len(features_np.shape) == 3:
                        # 对于序列特征，使用平均池化得到 [B, C]
                        features_np = features_np.mean(axis=1)  # [B, N, C] -> [B, C]
                    elif len(features_np.shape) == 2:
                        # 已经是2D，直接使用
                        pass
                    else:
                        # 其他情况，展平
                    ...
                    # 如果是列表或其他类型，转换为numpy数组
                    if isinstance(features, list):
                        # 如果是多尺度特征列表，取第一个并处理
                        feat = features[0] if features else None
                        if feat is not None:
                            feat_np = feat.cpu().numpy() if isinstance(feat, torch.Tensor) else np.array(feat)
