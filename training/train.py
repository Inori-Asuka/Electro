...
def train(cfg: DictConfig):
    ...
    model = create_dinov3_model(
        ...
        model_name=cfg.dinov3.get('model_name', 'resnet50'),  # 用于 vit, resnet 和 swin-t
        pretrained=cfg.dinov3.get('pretrained', True),  # 用于 vit, resnet 和 swin-t
        num_outputs=1,
    ...
    # 显示模型信息
    backbone_type = cfg.dinov3.get('backbone_type', 'convnext')
    model_name = cfg.dinov3.get('model_name', '')
    
    if backbone_type == 'convnext':
        model_display = 'DINOv3 ConvNeXt-Large'
    elif backbone_type == 'dinov3-vit':
        model_display = 'DINOv3 ViT Large'
    elif backbone_type == 'vit':
        model_display = f'ViT ({model_name})' if model_name else 'ViT'
    elif backbone_type == 'resnet':
        model_display = f'ResNet ({model_name})' if model_name else 'ResNet'
    elif backbone_type == 'swin-t':
        model_display = f'Swin Transformer ({model_name})' if model_name else 'Swin Transformer'
    else:
        model_display = f'{backbone_type} ({model_name})' if model_name else backbone_type
